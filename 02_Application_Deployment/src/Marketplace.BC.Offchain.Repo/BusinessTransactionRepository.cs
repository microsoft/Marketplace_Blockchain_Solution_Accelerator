...
namespace Marketplace.BC.Offchain.Repo
{
    public class BusinessTransactionRepository<TEntity, TIdentifier> : IRepository<TEntity, TIdentifier> where TEntity : class, IEntityModel<TIdentifier>
    {
        ...
        public BusinessTransactionRepository(IMongoClient client)
        {
            ...
            if (!BsonClassMap.IsClassMapRegistered(typeof(SampleDTO))) 
                BsonClassMap.RegisterClassMap<SampleDTO>();

        }

        public BusinessTransactionRepository()
        {
        }
        ...
        {
            return _database.GetCollection<Vendors>("vendors").Find("{}").ToList();
        }
